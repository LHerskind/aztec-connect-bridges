...
export class RollupProcessor {
	private contract: MockRollupProcessor;
	private uniswapContract: ISwapRouter;
	private WETH9: string;
	private wethContract: WETH;

	constructor(public address: string, signer: Signer) {
		this.contract = MockRollupProcessor__factory.connect(address, signer);
		this.WETH9 = "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2";

		this.uniswapContract = ISwapRouter__factory.connect("0xe592427a0aece92de3edee1f18e0157c05861564", signer);
		this.wethContract = WETH__factory.connect(this.WETH9, signer);
	}

	static async deploy(signer: Signer, bridgeProxyAddress: string) {
		const contract = await (await new MockRollupProcessor__factory(signer).deploy(bridgeProxyAddress)).deployed();
    ...
    */
	...
	async processAsyncDefiInteraction(signer: Signer, interactionNonce: BigNumber, options: SendTxOptions = {}) {
		const contract = new Contract(this.contract.address, this.contract.interface, signer);
		const { gasLimit, gasPrice } = options;
	...
	async preFundContractWithToken(signer: Signer, token: TestToken, to?: string) {
		const amount = parseUnits("1", 21);
