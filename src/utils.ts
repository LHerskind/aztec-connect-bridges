...
export async function getTokenBalance(tokenAddress: string, address: string, signer: Signer) {
	const tokenContract = new Contract(tokenAddress, ERC20.abi, signer);
	return await tokenContract.balanceOf(address);
}

export async function approveToken(tokenAddress: string, spender: string, signer: Signer, amount: BigNumber) {
	const tokenContract = new Contract(tokenAddress, ERC20.abi, signer);
	const approved = await tokenContract.approve(spender, amount);
	await approved.wait();
}

export async function transferToken(tokenAddress: string, recipient: string, signer: Signer, amount: BigNumber) {
	const tokenContract = new Contract(tokenAddress, ERC20.abi, signer);
...
export async function approveWeth(spender: string, amount: BigNumber, signer: Signer) {
...
export async function depositToWeth(amount: BigNumber, signer: Signer) {
	const wethContract = createWethContract(signer);
	const address = await signer.getAddress();
	const balance = await wethContract.balanceOf(address);
	if (balance < amount) {
		const amountToAdd = amount.sub(balance);
